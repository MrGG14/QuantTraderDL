...
class SVMWavelet:
    ...
    # def forecast(self, data, model='svm', threshold = 0.5):
    #     """
    ...
    #             std_threshold = np.std(coeffs[i]) * 0.5  # Filtrar coeficientes por desviación estándar
    #             coeffs[i] = pywt.threshold(coeffs[i], std_threshold)
    #         if model == 'svm':
    #             forecasted = self._svm_forecast(coeffs[i])
    #         elif model == 'lstm':
    #             forecasted = self._lstm_forecast(coeffs[i])
    ...
    def forecast(self, data, model='svm', threshold = 0.5):
        ...
        if model == 'svm':
            ...
            return self._lstm_forecast(data)
    ...
    # Just for testing
    def _lstm_forecast(self, data, sample_size=10):
        X, y = self._partition_array(data, size=sample_size)

        model = Sequential([
            LSTM(50, activation='relu', input_shape=(sample_size, 1)),
            Dense(1)
        ])
        model.compile(optimizer='adam', loss='mse')

        model.fit(X.reshape(-1, sample_size, 1), y, epochs=100, verbose=0)
        return model.predict(data[np.newaxis, -sample_size:].reshape(1, sample_size, 1))[0, 0]
